...
public class PantallaJuego implements Screen{
	...
	public void render(float delta) {
        ...
        if((entradas.isIzquierda()||entradas.isA()) && player.spr.getX() > 40) {
        	player.setX(player.getX() - 3);
        }
        if((entradas.isDerecha()||entradas.isD()) && player.spr.getX() < 1240 - player.spr.getWidth()) {
        	player.setX(player.getX() + 3);
        }
        if((entradas.isArriba()||entradas.isW()) && player.spr.getY() < 672 - player.spr.getHeight()) {
        	player.setY(player.getY() + 3);
        }
        if((entradas.isAbajo()||entradas.isS()) && player.spr.getY() > 48) {
        	player.setY(player.getY() - 3);
        }
	}
